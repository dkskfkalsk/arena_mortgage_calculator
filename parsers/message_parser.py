...
class MessageParser:
    ...
    def parse(self, message_text: str) -> Dict[str, Any]:
        ...
        data = {
            ...
            "occupation": None,
            "credit_score": None,
            "credit_score_raw": None,  # 원본 신용점수/등급 문자열 (MG캐피탈 등급 파싱용)
            "residence": None,
            "ownership": None,
            "address": None,
            "area": None,
        ...
        if data["requests"]:
            ...
            # 1. 억 단위 패턴
            required_match = re.search(rf'(?:{_req_keywords})[:\s]*(\d+(?:[.,]\d+)?)\s*억', req_text)
            if required_match:
                amount_eok = float(required_match.group(1).replace(",", "").replace(".", ""))
                data["required_amount"] = amount_eok * 10000
                print(f"DEBUG: Parsed required_amount from 억: {data['required_amount']}만원")
            else:
                # 2. 천만 단위 패턴
                required_match = re.search(rf'(?:{_req_keywords})[:\s]*(\d+)\s*천만', req_text)
                if required_match:
            ...
            if "전체 대환" in data["requests"]:
                ...
                if "대환" in data["requests"]:
                    ...
                    if range_match or hyphen_match:
                        ...
                        end_priority = int(m.group(2))
                        print(f"DEBUG: Found 'N~M순위/N-M순위 대환' pattern - priority range: {start_priority}~{end_priority}, treating as refinance request")
                        for mortgage in data["mortgages"]:
                        ...
                        if len(priorities) >= 2:
                            ...
                            print(f"DEBUG: Found '1, 2순위 대환' pattern - priorities: {priority_set}, treating as refinance request")
                            for mortgage in data["mortgages"]:
                                priority = mortgage.get("priority")
        ...
        if not has_refinance:
            ...
            if range_match or hyphen_match:
                ...
                start_priority = int(m.group(1))
                end_priority = int(m.group(2))
                print(f"DEBUG: Found 'N~M순위/N-M순위 대환' pattern in full text - priority range: {start_priority}~{end_priority}, treating as refinance request")
                for mortgage in data["mortgages"]:
                    priority = mortgage.get("priority")
                    if priority and start_priority <= priority <= end_priority:
                        mortgage["is_refinance"] = True
                ...
                if len(priorities) >= 2:
                    priority_set = set(priorities)
                    print(f"DEBUG: Found '1, 2순위 대환' pattern in full text - priorities: {priority_set}, treating as refinance request")
                    for mortgage in data["mortgages"]:
                        priority = mortgage.get("priority")
                        if priority and priority in priority_set:
                            mortgage["is_refinance"] = True
            ...
            # 2. "1순위 [기관1], 2순위 [기관2] 대환조건" 패턴 (기관명이 사이에 있어도 됨)
            if not has_refinance:
    ...
    def _extract_kb_price_from_text(self, text: str) -> Optional[str]:
        ...
        # KB시세가 포함된 줄 찾기 (KB AI시세 제외)
        for i, line in enumerate(lines):
            line_lower = line.lower()
            line_compact = line_lower.replace(" ", "")
            if 'kbaise' in line_compact or 'kbai' in line_compact:
                continue  # KB AI시세는 별도 필드로 처리
    ...
    def _extract_region(self, address: str) -> Optional[str]:
        ...
        regions = [
            ...
            "인천광역시서해구청라동", "인천광역시서해구가정동", "인천광역시서해구신현동",
            "인천광역시서구", "인천광역시미추홀구", "인천광역시강화군", "인천광역시옹진군",
            "인천광역시제물포구만석동", "인천광역시제물포구화수동", "인천광역시제물포구송현동",
            "인천광역시제물포구화평동", "인천광역시제물포구창영동", "인천광역시제물포구송림동",
            "인천광역시제물포구금곡동",
            "인천광역시제물포구", "인천광역시영종구", "인천광역시서해구", "인천광역시검단구",
            "광주광역시동구", "광주광역시서구", "광주광역시남구", "광주광역시북구", "광주광역시광산구",
            "전남광주통합특별시동구", "전남광주통합특별시서구", "전남광주통합특별시남구",
            ...
            "전라남도진도군", "전라남도신안군",
            "전남광주통합특별시목포시", "전남광주통합특별시여수시", "전남광주통합특별시순천시",
            "전남광주통합특별시나주시", "전남광주통합특별시광양시", "전남광주통합특별시담양군",
            "전남광주통합특별시곡성군", "전남광주통합특별시구례군", "전남광주통합특별시고흥군",
            "전남광주통합특별시보성군", "전남광주통합특별시화순군", "전남광주통합특별시장흥군",
            "전남광주통합특별시강진군", "전남광주통합특별시해남군", "전남광주통합특별시영암군",
            "전남광주통합특별시무안군", "전남광주통합특별시함평군", "전남광주통합특별시영광군",
            "전남광주통합특별시장성군", "전남광주통합특별시완도군", "전남광주통합특별시진도군",
            "전남광주통합특별시신안군",
    ...
    def _extract_required_amount(self, requests_text: str) -> Optional[float]:
        ...
        if not re.search(req_keywords, requests_text):
            return None
        patterns = [
            (rf'(?:{req_keywords})\s*[:\s]*(\d+)\s*억', 10000),
            (rf'(?:{req_keywords})\s*[:\s]*(\d+)\s*천만', 1000),
            (rf'(?:{req_keywords})\s*[:\s]*([\d,]+)\s*만원?', 1),
            (rf'(?:{req_keywords})\s*[:\s]*([\d,]+)', 1),
        ]
        for pattern, multiplier in patterns:
            ...
            if match:
                ...
                try:
                    ...
                    print(f"DEBUG: _extract_required_amount - found: {amount}만원")
                    return amount
                except ValueError:
                    continue
        return None
