...
> **문서 성격**: 내부 개발 로드맵 (아직 전면 적용 전)  
> **목표**: 조견 JSON만 수정하면 모든 금융사에 동일 규칙이 적용되도록 스키마·config 플래그 통합

현재 운영 조견은 `data/banks/*.json`에 금융사별 키가 혼재합니다.  
이 문서는 `calculator/base_calculator.py`의 금융사명 분기를 줄이기 위한 설계안입니다.
